...
- [X] 1에서 9까지 서로 다른 3자리 수를 입력한다.
- [X] 입력한 수에 대한 힌트를 받는다.
- [X] 정답을 맞춘 경우 프로그램 종료 여부를 입력한다.

### 힌트

- [X] 같은 자리에 같은 숫자가 있는 경우 `스트라이크`이다.
- [X] 다른 자리에 숫자가 있는 경우 `볼`이다.
- [X] 같은 숫자가 하나도 없는 경우 `낫싱`이다.

### 종료

- [X] 유저가 입력한 3자리 숫자가 정답(프로그램의 수)과 같은 경우 게임이 종료된다.
...
- 유저의 비정상적인 입력에 대해서 IllegalArgumentException을 발생시킨다.
  - [X] 유저의 3자리 수에 입력에 대한 비정상적인 입력
    - 입력한 수의 길이가 3이 아닌 경우
    - 입력한 수가 1~9 이외의 값인 경우
    - 동일한 수가 중복 입력된 경우
  - [X] 유저의 프로그램 종료를 위한 입력에 대한 비정상적인 입력
    - `1`과 `2` 이외의 값이 입력된 경우
